...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	*/
	@Transactional(readOnly = false)
	public void updataUserBasicInfo(User user) {
		User oldUser = userDao.get(user.getId());
		if (oldUser.getOffice() != null && oldUser.getOffice().getId() != null){
			CacheUtils.remove(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + oldUser.getOffice().getId());
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
    ...
    public User getUserByOpenId(User user) {
		return  userDao.getUserByOpenId(user);
    }
	@Transactional(readOnly = false)
	public void updataPrecise(User user) {
		userDao.updataPrecise(user);
	}
	@Transactional(readOnly = false)
	public void updateExpOfLatLng(UserExp exp) {
		userDao.updateExpOfLatLng(exp);
	}
	@Transactional(readOnly = false)
	public void saveExp(UserExp exp) {
		userDao.saveExp(exp);
		UserUtils.clearCache(UserUtils.getUser());

	}

    public List<Map<String, Object>> findAllCheerData(String type) {
		return userDao.findAllCheerData(type);
    }

    public List<UserExp> findCheerList() {
		return userDao.findCheerList();
    }

    public Integer getcount(String type) {
		return userDao.getCount(type);
    }

	public List<Map<String, Object>> getCountByCha(String type) {
		return userDao.getCountByCha(type);
	}

    public List<Map<String, Object>> findTopCheerData(String type) {
		return userDao.findTopCheerData(type);
    }
